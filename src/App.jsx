...
    if (planElement) {
        // Aggiunge un feedback visivo e disabilita il pulsante
        const originalButtonText = document.querySelector('button[onClick*="exportPlanToJpeg"]').innerText;
        document.querySelector('button[onClick*="exportPlanToJpeg"]').innerText = 'Esportazione...';
        document.querySelector('button[onClick*="exportPlanToJpeg"]').disabled = true;

        // Clona l'elemento per la cattura e lo inserisce temporaneamente fuori schermo
...
        // Introduce un leggero ritardo per assicurare che il DOM sia pronto
        setTimeout(() => {
...
            document.querySelector('button[onClick*="exportPlanToJpeg"]').innerText = originalButtonText;
            document.querySelector('button[onClick*="exportPlanToJpeg"]').disabled = false;
          });
        }, 50); // Ritardo di 50ms
    }
  };
...
              <textarea id="avoid-ingredients" value={avoidIngredients} onChange={(e) => setAvoidIngredients(e.target.value)} className="w-full bg-gray-700 text-white border border-gray-600 rounded-lg p-3 h-24 focus:ring-2 focus:focus:ring-red-500 focus:outline-none" placeholder="Es: cipolla, aglio..."/>
            </div>
            <div>
